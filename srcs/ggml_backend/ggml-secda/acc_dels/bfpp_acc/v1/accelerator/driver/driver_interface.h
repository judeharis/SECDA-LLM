...
void updatePlan(int supported_nodes) {
  ...
  drv->m.planned = true;
  drv->t.alloced_buffer_ids.resize(supported_nodes);
  for (int i = 0; i < supported_nodes; i++) {
    drv->t.alloced_buffer_ids[i] = -1;
  }
}
...
static bool preloadWeights(unsigned wgt_size, int layer, int M, int K,
  ...
  bool alloced = drv->t.alloc_layer(layer, wgt_size);
  if (alloced) {
    char *DMA_W = reinterpret_cast<char *>(drv->DMA_W);
    acc_block_q6_K *wgt_block_6 = (acc_block_q6_K *)wgt;
    acc_block_q5_K *wgt_block_5 = (acc_block_q5_K *)wgt;
    acc_block_q4_K *wgt_block_4 = (acc_block_q4_K *)wgt;
    acc_block_q3_K *wgt_block_3 = (acc_block_q3_K *)wgt;
    acc_block_q2_K *wgt_block_2 = (acc_block_q2_K *)wgt;
    uint32_t wgt_blck = 0;
    ...
    for (int m = 0; m < M; m++) {
      for (int k = 0; k < (K / 256); k++) {
        int *wgt_block_6_ptr = (int *)&wgt_block_6[m * (K / 256) + k];
        int *wgt_block_5_ptr = (int *)&wgt_block_5[m * (K / 256) + k];
        int *wgt_block_4_ptr = (int *)&wgt_block_4[m * (K / 256) + k];
        int *wgt_block_3_ptr = (int *)&wgt_block_3[m * (K / 256) + k];
        int *wgt_block_2_ptr = (int *)&wgt_block_2[m * (K / 256) + k];

        if (wgt_type == 6)
          memcpy(&DMA_W[drv->t.curr_offset], wgt_block_6_ptr, wgt_blck * 4);
        if (wgt_type == 5)
          memcpy(&DMA_W[drv->t.curr_offset], wgt_block_5_ptr, wgt_blck * 4);
        if (wgt_type == 4)
          memcpy(&DMA_W[drv->t.curr_offset], wgt_block_4_ptr, wgt_blck * 4);
        if (wgt_type == 3)
          memcpy(&DMA_W[drv->t.curr_offset], wgt_block_3_ptr, wgt_blck * 4);
        if (wgt_type == 2)
          memcpy(&DMA_W[drv->t.curr_offset], wgt_block_2_ptr, wgt_blck * 4);

        drv->t.curr_offset += wgt_blck * 4;
      }
    }
  }
...
static void initACC() {
  ...
  if (!dparams.init) {
...
#endif
#endif
    std::cout << "BFPP Accelerator Driver Version 1";
#ifdef ACC_NEON
    std::cout << " with Neon";
#endif
    std::cout << std::endl;
    ...
    drv->DMA_I = drv->mdma->dmas[0].dma_get_inbuffer();
    drv->DMA_W = drv->DMA_I + (DMA_INP_SIZE / 4);
