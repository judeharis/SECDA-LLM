...
void LoadWeights_Inference(acc_container *drv, int m, int mstep, int nstep,
  ...
  uint32_t wgt_blck = 0;
  acc_block_q6_K *wgt_block_6 = (acc_block_q6_K *)wgt_block;
  acc_block_q5_K *wgt_block_5 = (acc_block_q5_K *)wgt_block;
  acc_block_q4_K *wgt_block_4 = (acc_block_q4_K *)wgt_block;
  acc_block_q3_K *wgt_block_3 = (acc_block_q3_K *)wgt_block;
  acc_block_q2_K *wgt_block_2 = (acc_block_q2_K *)wgt_block;
  ...
  for (int ms = m; ms < (m + mstep); ms++) {
    for (int k = 0; k < kb; k++) {
      int *wgt_block_6_ptr = (int *)&wgt_block_6[ms * kb + k];
      int *wgt_block_5_ptr = (int *)&wgt_block_5[ms * kb + k];
      int *wgt_block_4_ptr = (int *)&wgt_block_4[ms * kb + k];
      int *wgt_block_3_ptr = (int *)&wgt_block_3[ms * kb + k];
      int *wgt_block_2_ptr = (int *)&wgt_block_2[ms * kb + k];

      if (drv->wgt_type == 6) memcpy(&DMA_I[ld], wgt_block_6_ptr, wgt_blck * 4);
      if (drv->wgt_type == 5) memcpy(&DMA_I[ld], wgt_block_5_ptr, wgt_blck * 4);
      if (drv->wgt_type == 4) memcpy(&DMA_I[ld], wgt_block_4_ptr, wgt_blck * 4);
      if (drv->wgt_type == 3) memcpy(&DMA_I[ld], wgt_block_3_ptr, wgt_blck * 4);
      if (drv->wgt_type == 2) memcpy(&DMA_I[ld], wgt_block_2_ptr, wgt_blck * 4);
      ld += wgt_blck;
    }
  }
...
void LoadWeights_Preloaded(acc_container *drv, int m, int mstep, int nstep,
  ...
  // Send Weight Data
  prf_start(6);
  unsigned int layer_offset = drv->t.layer_offsets[drv->t.layer] + DMA_INP_SIZE;
  drv->mdma->dmas[0].dma_change_start(layer_offset + ((wgt_blck * 4) * m * kb));
  ...
  TOG2("Send Compute OP\n");
  prf_start(2);
  drv->mdma->dmas[0].dma_change_start(0);
  drv->mdma->dmas[0].dma_start_send(ld);
  drv->mdma->dmas[0].dma_wait_send();
  prf_end(2, drv->a_t->fpga_compute);
  TOG2("Compute Op Sent\n");
}
