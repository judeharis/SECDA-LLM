...
// Macro Space
// ============
#define ARCH_BFPP_QK2 1
#define ARCH_BFPP_QK3 1
#define ARCH_BFPP_QK4 1
#define ARCH_BFPP_QK5 1
#define ARCH_BFPP_QK6 1
...
struct wgt_packet {
  ...
  wgt_packet(sc_fifo_in<ADATA> *din) {
    ...
    a_size = din->read().data;
    b_size = din->read().data;
    c_size = din->read().data;
    d_size = din->read().data;
  }
};
...
struct BFPP_vars {
  ...
  DEFINE_SC_SIGNAL(bool, load_inp);
  DEFINE_SC_SIGNAL(bool, load_wgt);
  DEFINE_SC_SIGNAL(bool, bfpp_start);
  DEFINE_SC_SIGNAL(bool, bfpp_ready);
  DEFINE_SC_SIGNAL(unsigned int, kb);
  DEFINE_SC_SIGNAL(unsigned int, wbs);
  DEFINE_SC_SIGNAL(unsigned int, ibs);
  ...
  sc_fifo<ADATA> wgt_fifo2;
  sc_fifo<ADATA> wgt_fifo3;
...
#pragma HLS resource variable = wgt_fifo2 core = FIFO_SRL
#pragma HLS resource variable = wgt_fifo3 core = FIFO_SRL
